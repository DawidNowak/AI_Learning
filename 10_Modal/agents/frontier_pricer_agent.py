...
class FrontierAgent(Agent):
    ...
    def __init__(self, collection):
        """
        Set up this instance by connecting to OpenAI and the Chroma Datastore,
        And setting up the vector encoding model
        """
        self.log(f"Initializing {self.name}")
        self.client = OpenAI()
        self.MODEL = "gpt-4o-mini"
        self.log(f"{self.name} is set up with OpenAI")
    ...
    def messages_for(self, description: str, similars: List[str], prices: List[float]) -> List[Dict[str, str]]:
        ...
        :param description: a description of the product to estimate
    ...
    def find_similars(self, description: str):
        ...
        """
        self.log(f"{self.name} is performing a RAG search of the Chroma datastore to find 5 similar products")
        vector = self.model.encode([description])
        results = self.collection.query(query_embeddings=vector.astype(float).tolist(), n_results=5)
        documents = results['documents'][0][:]
        prices = [m['price'] for m in results['metadatas'][0][:]]
        self.log(f"{self.name} has found similar products")
    ...
    def price(self, description: str) -> float:
        ...
        self.log(f"{self.name} is about to call {self.MODEL} with context including 5 similar products")
